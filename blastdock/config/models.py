...
class DefaultPortsConfig(BaseModel):
    ...
    wordpress: int = Field(
        default=8080, ge=1024, le=65535, description="WordPress default port"
    )
    mysql: int = Field(default=3306, ge=1, le=65535, description="MySQL default port")
    postgresql: int = Field(
        default=5432, ge=1, le=65535, description="PostgreSQL default port"
    )
    ...
    grafana: int = Field(
        default=3000, ge=1024, le=65535, description="Grafana default port"
    )
    prometheus: int = Field(
        default=9090, ge=1024, le=65535, description="Prometheus default port"
    )
    traefik: int = Field(
        default=8080, ge=1024, le=65535, description="Traefik dashboard port"
    )
    portainer: int = Field(
        default=9000, ge=1024, le=65535, description="Portainer default port"
    )
...
class LoggingConfig(BaseModel):
    ...
    backup_count: int = Field(
        default=5, ge=1, le=100, description="Number of backup log files"
    )
...
class DockerConfig(BaseModel):
    ...
    default_restart_policy: RestartPolicy = Field(
        default=RestartPolicy.UNLESS_STOPPED, description="Default restart policy"
    )
    timeout: int = Field(
        default=30, ge=5, le=600, description="Docker command timeout in seconds"
    )
    max_retries: int = Field(
        default=3, ge=1, le=10, description="Maximum retry attempts"
    )
    build_timeout: int = Field(
        default=600, ge=60, le=3600, description="Build timeout in seconds"
    )
    pull_timeout: int = Field(
        default=300, ge=30, le=1800, description="Pull timeout in seconds"
    )
...
class SecurityConfig(BaseModel):
    ...
    auto_generate_passwords: bool = Field(
        default=True, description="Auto-generate secure passwords"
    )
    password_length: int = Field(
        default=16, ge=12, le=128, description="Generated password length"
    )
    password_complexity: bool = Field(
        default=True, description="Require complex passwords"
    )
    confirm_destructive_operations: bool = Field(
        default=True, description="Confirm destructive operations"
    )
    require_port_confirmation: bool = Field(
        default=True, description="Confirm port assignments"
    )
    enable_secrets_encryption: bool = Field(
        default=True, description="Encrypt stored secrets"
    )
    secrets_key_rotation_days: int = Field(
        default=90, ge=30, le=365, description="Secret key rotation interval"
    )
    audit_sensitive_operations: bool = Field(
        default=True, description="Audit sensitive operations"
    )
    max_failed_attempts: int = Field(
        default=5, ge=3, le=20, description="Max failed authentication attempts"
    )
    ...
    def validate_password_length(cls, v):
        ...
        if v < 12:
            raise ValueError(
                "Password length must be at least 12 characters for security"
            )
        return v
...
class TemplateConfig(BaseModel):
    ...
    update_check_interval: int = Field(
        default=86400, ge=3600, description="Update check interval in seconds"
    )
    custom_template_dirs: List[str] = Field(
        default_factory=list, description="Custom template directories"
    )
    template_cache_ttl: int = Field(
        default=3600, ge=60, description="Template cache TTL in seconds"
    )
    enable_template_validation: bool = Field(
        default=True, description="Enable template validation"
    )
    allow_insecure_templates: bool = Field(
        default=False, description="Allow templates with security warnings"
    )
    max_template_size: int = Field(
        default=50 * 1024 * 1024, description="Max template size in bytes"
    )
    ...
    def validate_template_dirs(cls, v):
        ...
        for dir_path in v:
            if not os.path.isabs(dir_path):
                raise ValueError(
                    f"Template directory must be absolute path: {dir_path}"
                )
...
class PerformanceConfig(BaseModel):
    ...
    cache_strategy: CacheStrategy = Field(
        default=CacheStrategy.HYBRID, description="Caching strategy"
    )
    max_memory_cache_size: int = Field(
        default=128 * 1024 * 1024,
        ge=16 * 1024 * 1024,
        description="Max memory cache size in bytes",
    )
    max_disk_cache_size: int = Field(
        default=1024 * 1024 * 1024,
        ge=100 * 1024 * 1024,
        description="Max disk cache size in bytes",
    )
    ...
    parallel_operations: int = Field(
        default=4, ge=1, le=16, description="Number of parallel operations"
    )
...
class MonitoringConfig(BaseModel):
    ...
    alert_threshold: int = Field(
        default=3, ge=1, le=10, description="Alert threshold for consecutive failures"
    )
...
class NetworkConfig(BaseModel):
    ...
    default_network_driver: str = Field(
        default="bridge", description="Default network driver"
    )
    ...
    subnet_prefix: str = Field(
        default="172.20.0.0/16", description="Default subnet prefix"
    )
    enable_network_isolation: bool = Field(
        default=True, description="Enable network isolation"
    )
    default_domain: str = Field(
        default="localhost", description="Default domain for deployments"
    )
...
class BackupConfig(BaseModel):
    ...
    max_backup_count: int = Field(
        default=7, ge=1, le=100, description="Maximum backup count"
    )
    ...
    remote_backup_enabled: bool = Field(
        default=False, description="Enable remote backup"
    )
    remote_backup_provider: Optional[str] = Field(
        default=None, description="Remote backup provider"
    )
...
class BlastDockConfig(BaseModel):
    ...
    def validate_config_compatibility(cls, values):
        ...
        if version:
            ...
            if major_version != "1":
                logger.warning(
                    f"Configuration version {version} may not be fully compatible"
                )
