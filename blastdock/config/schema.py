...
class ConfigSchema:
    ...
    def validate_config_against_schema(
        self, config: Dict[str, Any], schema_name: str = "blastdock-config"
    ) -> List[str]:
    ...
    def validate_partial_config(
        self, config: Dict[str, Any], section: str
    ) -> List[str]:
    ...
    def _extract_property_docs(
        ...
        for prop_name, prop_schema in properties.items():
            ...
            if prop_schema.get("type") == "object" and "properties" in prop_schema:
                docs[prop_name]["properties"] = self._extract_property_docs(
    ...
    def create_config_template(
        self, schema_name: str = "blastdock-config"
    ) -> Dict[str, Any]:
...
class ConfigValidator:
    ...
    def _run_custom_validations(self, config: Dict[str, Any]) -> List[str]:
        ...
        if "docker" in config:
            ...
            if "compose_version" in docker_config:
                if not self._validate_docker_version(docker_config["compose_version"]):
                    issues.append(
                        f"Invalid Docker Compose version: {docker_config['compose_version']}"
                    )
        ...
        if "security" in config:
            ...
            if "password_length" in security_config:
                if not self._validate_positive_integer(
                    security_config["password_length"], min_val=8
                ):
    ...
    def _validate_cross_sections(self, config: Dict[str, Any]) -> List[str]:
        ...
        if monitoring.get("enable_metrics", True):
            ...
            if retention_days < 7:
                issues.append(
                    "Metrics retention should be at least 7 days when monitoring is enabled"
                )
        ...
        if performance.get("cache_strategy") == "memory":
            ...
            if max_memory < 16 * 1024 * 1024:  # 16MB
                issues.append(
                    "Memory cache size is too small for memory-only caching strategy"
                )
    ...
    def _validate_directory_path(self, path: Any) -> bool:
        ...
        try:
            ...
            return path_obj.is_absolute() and not any(
                char in str(path_obj) for char in ["<", ">", "|", "*", "?"]
            )
        except Exception:
            return False
        ...
        if section in config:
            ...
            if section == "default_ports":
                for port_name, port_value in section_config.items():
                    if not self._validate_port_range_custom(port_value):
                        issues.append(
                            f"Invalid port value for {port_name}: {port_value}"
                        )

            elif section == "logging":
                if "level" in section_config and not self._validate_log_level(
                    section_config["level"]
                ):
                ...
                if "password_length" in section_config:
                    if not self._validate_positive_integer(
                        section_config["password_length"], min_val=8
                    ):
                        issues.append("Password length must be at least 8 characters")
    ...
    def get_validation_suggestions(self, config: Dict[str, Any]) -> List[str]:
        ...
        if "backup" not in config:
            suggestions.append(
                "Consider enabling backup configuration for data protection"
            )
        ...
        if not security_config.get("enable_secrets_encryption", True):
            suggestions.append(
                "Consider enabling secrets encryption for better security"
            )
