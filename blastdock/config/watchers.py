...
class ConfigWatcher:
    ...
    def __init__(self, config_file: Path, check_interval: float = 1.0):
        ...
        self._use_polling = (
            True  # Use polling by default for cross-platform compatibility
        )
            ...
            class ConfigFileHandler(FileSystemEventHandler):
                ...
                def on_modified(self, event):
                    if (
                        ...
                        and Path(event.src_path) == self.watcher.config_file
                    ):
    ...
    def _get_file_checksum(self) -> str:
        ...
        try:
            ...
            logger.error(
                f"Unexpected error calculating checksum for {self.config_file}: {e}"
            )
            return ""
    ...
    def _update_file_state(self) -> None:
        ...
        if self.config_file.exists():
            try:
                ...
                logger.warning(
                    f"OS error updating file state for {self.config_file}: {e}"
                )
                ...
                # BUG-007 FIX: Log unexpected errors
                logger.error(
                    f"Unexpected error updating file state for {self.config_file}: {e}"
                )
                self._last_modified = None
                self._last_size = None
                self._last_checksum = None
    ...
    def _handle_file_change(self) -> None:
        ...
        if (
            ...
            and (current_time - self._last_change_time) < self._debounce_delay
        ):
            return
...
class MultiConfigWatcher:
    ...
    def add_config_file(
        ...
        name: str,
        config_file: Path,
        callbacks: Optional[List[Callable[[Path], None]]] = None,
    ) -> None:
    ...
    def list_watched_files(self) -> Dict[str, str]:
        ...
        return {
            name: str(watcher.config_file) for name, watcher in self._watchers.items()
        }
...
class ConfigChangeLogger:
    ...
    def log_change(
        self, config_name: str, file_path: Path, change_type: str = "modified"
    ) -> None:
