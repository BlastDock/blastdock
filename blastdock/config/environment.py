    ...
    def _flatten_config_to_env(
        ...
        for key, value in config.items():
            ...
            if isinstance(value, dict):
                ...
                nested_vars = self._flatten_config_to_env(
                    value, f"{prefix}{key.upper()}_"
                )
        ...
        if "security" in config:
            ...
            if length is not None:
                if not isinstance(length, int) or length < 12:
                    issues.append(
                        "BLASTDOCK_SECURITY_PASSWORD_LENGTH must be an integer >= 12"
                    )
    ...
    def create_docker_env_file(self, config: Dict[str, Any], output_path: str) -> None:
        ...
        if "environment_variables" in config:
            ...
            if isinstance(env_vars, dict):
                ...
                self.logger.warning(
                    f"environment_variables is not a dict, got {type(env_vars).__name__}"
                )
        ...
        if "default_ports" in config:
            ...
            if isinstance(ports, dict):
                ...
                self.logger.warning(
                    f"default_ports is not a dict, got {type(ports).__name__}"
                )
