...
class ConfigManager:
        ...
        try:
            with self._config_lock:
                ...
                if validation_errors:
                    logger.warning(
                        f"Configuration validation issues: {validation_errors}"
                    )
            ...
            if legacy_path.exists():
                ...
                try:
                    ...
                    self.backup_manager.create_backup(
                        legacy_config, description=f"Legacy config from {legacy_path}"
                    )

                    return migrated_config
                except Exception as e:
                    logger.error(
                        f"Failed to migrate legacy config from {legacy_path}: {e}"
                    )
    ...
    def save_config(self, config: Optional[BlastDockConfig] = None) -> None:
        ...
        try:
            with self._config_lock:
                ...
                try:
                    current_config = self.persistence.load_config(
                    ...
                    logger.debug(
                        f"No existing config to backup for profile '{self.profile}'"
                    )
    ...
    def update_settings(
        self, settings: Dict[str, Any], save: Optional[bool] = None
    ) -> None:
    ...
    def reset_to_defaults(self, sections: Optional[List[str]] = None) -> None:
        ...
        try:
            with self._config_lock:
                ...
                logger.info(
                    f"Reset configuration to defaults: {sections or 'all sections'}"
                )
    ...
    def switch_profile(self, profile_name: str) -> None:
        ...
        if self.auto_save and self._config is not None:
            try:
                ...
                logger.error(
                    f"Failed to save current profile '{self.profile}' before switching: {e}"
                )
...
def get_config_manager(
    profile: str = "default", auto_save: bool = True, watch_changes: bool = False
) -> ConfigManager:
    ...
    with _manager_lock:
        if (
            ...
            or _config_manager.profile != profile
            or _config_manager.auto_save != auto_save
            or _config_manager.watch_changes != watch_changes
        ):
