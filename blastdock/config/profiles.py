...
class ProfileManager:
    ...
    def copy_profile(
        ...
        if self.profile_exists(target_profile):
            raise ConfigurationError(
                f"Target profile '{target_profile}' already exists"
            )
    ...
    def export_profile(
        self, profile_name: str, export_path: str, include_metadata: bool = True
    ) -> None:
    ...
    def import_profile(
        self, import_path: str, profile_name: str, description: Optional[str] = None
    ) -> None:
            ...
            for other_profile in self._profile_cache.keys():
                if other_profile != profile_name:
                    try:
                        ...
                        if (
                            other_metadata.get("base_profile") == profile_name
                            or other_metadata.get("copied_from") == profile_name
                        ):
    ...
    def get_profile_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "oldest_profile": (
                {"name": oldest.name, "created_at": oldest.created_at.isoformat()}
                if oldest
                else None
            ),
            "newest_profile": (
                {"name": newest.name, "created_at": newest.created_at.isoformat()}
                if newest
                else None
            ),
            "profiles_with_metadata": len(
                [p for p in profiles if self._has_metadata(p.name)]
            ),
