...
def status(output_format, detailed):
    ...
    cache_status = (
        "🟢 Good" if hit_rate > 80 else "🟡 Fair" if hit_rate > 60 else "🔴 Poor"
    )
    ...
    mem_status = (
        "🟢 Good"
        if memory_util < 80
        else "🟡 High" if memory_util < 95 else "🔴 Critical"
    )
    ...
    template_status = (
        "🟢 Good"
        if template_hit_rate > 70
        else "🟡 Fair" if template_hit_rate > 50 else "🔴 Poor"
    )
    perf_table.add_row(
        "Templates", "Hit Rate", f"{template_hit_rate:.1f}%", template_status
    )
    ...
    load_status = (
        "🟢 Fast"
        if avg_load_time < 1
        else "🟡 Slow" if avg_load_time < 5 else "🔴 Very Slow"
    )
    perf_table.add_row(
        "Templates", "Avg Load Time", f"{avg_load_time:.2f}s", load_status
    )
    ...
    memory_status = (
        ...
        else "🟡 High" if memory_percent < 85 else "🔴 Critical"
    )
    perf_table.add_row(
        "Memory", "Usage", f"{memory_usage.get('rss_mb', 0):.1f}MB", memory_status
    )
    ...
    deploy_status = (
        "🟢 Fast"
        if avg_deployment_time < 30
        else "🟡 Slow" if avg_deployment_time < 120 else "🔴 Very Slow"
    )
    perf_table.add_row(
        "Deployment", "Avg Time", f"{avg_deployment_time:.1f}s", deploy_status
    )
    ...
    parallel_status = (
        "🟢 Good"
        if success_rate > 95
        else "🟡 Fair" if success_rate > 90 else "🔴 Poor"
    )
    perf_table.add_row(
        "Parallel", "Success Rate", f"{success_rate:.1f}%", parallel_status
    )

    console.print(perf_table)

    if detailed:
        _show_detailed_metrics(
            cache_stats,
            template_stats,
            deployment_stats,
            memory_usage,
...
@click.option(
    "--component",
    type=click.Choice(["cache", "memory", "templates", "deployment", "all"]),
    default="all",
...
@click.option(
    "--suite",
    type=click.Choice(["quick", "full", "system"]),
    default="quick",
...
def trends(operation, window):
    ...
    if operation:
        ...
        if trend_data["trend"] == "insufficient_data":
            console.print(
                f"[yellow]Insufficient data for {operation} (need at least {window} samples)[/yellow]"
            )
            return

        trend_color = {"improving": "green", "stable": "blue", "degrading": "red"}.get(
            trend_data["trend"], "white"
        )

        console.print(f"[bold]Operation:[/bold] {operation}")
        console.print(
            f"[bold]Trend:[/bold] [{trend_color}]{trend_data['trend'].title()}[/{trend_color}]"
        )
        console.print(
            f"[bold]Performance Change:[/bold] {trend_data['performance_change_percent']:+.1f}%"
        )
        console.print(
            f"[bold]Memory Change:[/bold] {trend_data['memory_change_percent']:+.1f}%"
        )
        ...
        trends_table = Table(
            title="Performance Trends", show_header=True, header_style="bold magenta"
        )
        ...
        for operation_name, profile in profiles.items():
            ...
            last_updated = time.strftime(
                "%H:%M:%S", time.localtime(profile.last_updated)
            )
...
def monitor(start_monitoring, stop_monitoring, interval):
    ...
    if start_monitoring:
        ...
        console.print(
            "[dim]Use 'blastdock performance monitor --stop-monitoring' to stop[/dim]"
        )
        ...
        status_table.add_row(
            "Monitoring Enabled", "✅ Yes" if metrics["monitoring_enabled"] else "❌ No"
        )
        status_table.add_row(
            "Monitoring Interval", f"{metrics['monitoring_interval']}s"
        )
...
def report(check_thresholds, export_report):
    ...
    if check_thresholds:
        ...
        if threshold_results["total_issues"] == 0:
            console.print(
                "[green]✅ All performance metrics within acceptable thresholds[/green]"
            )
        else:
            console.print(
                f"[yellow]⚠️ Found {threshold_results['total_issues']} performance issues[/yellow]\\n"
            )
    ...
    if export_report:
        ...
        report_data = {
            ...
            "profiles": {
                name: prof.__dict__
                for name, prof in benchmarks.get_all_profiles().items()
            },
        }
...
def _show_detailed_metrics(
    ...
    # Cache details
    cache_panel_content = []
    cache_panel_content.append(
        f"Memory Entries: {cache_stats.get('memory_entries', 0)}"
    )
    ...
    memory_panel_content.append(
        f"Virtual Memory: {memory_usage.get('vms_mb', 0):.1f}MB"
    )
    memory_panel_content.append(
        f"Python Objects: {memory_usage.get('python_objects', 0):,}"
    )
    memory_panel_content.append(
        f"Tracked Objects: {memory_usage.get('tracked_objects', 0)}"
    )
    memory_panel_content.append(
        f"Available Memory: {memory_usage.get('available_mb', 0):.1f}MB"
    )

    console.print(
        Panel(
            "\\n".join(memory_panel_content),
