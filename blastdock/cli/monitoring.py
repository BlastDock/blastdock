        ...
        if services:
            ...
            if detailed:
                ...
                for service_name, service_info in services.items():
                    ...
                    detail_text = Text()
                    detail_text.append(
                        f"Container: {container_info.get('name', 'unknown')}\n",
                        style="cyan",
                    )
                    detail_text.append(
...
def metrics(project_name, window, output_format):
    ...
    try:
        ...
        # Get dashboard data
        dashboard_data = metrics_collector.get_project_dashboard_data(
            project_name, window
        )
        ...
        for metric_name, display_name in key_metrics:
            if metric_name in metrics_data:
                ...
                overview_table.add_row(
                    display_name, current_str, avg_str, max_str, min_str
                )
        ...
        if containers:
            ...
            for container_name, container_metrics in containers.items():
                ...
                container_table.add_row(
                    container_name, f"{cpu_avg:.1f}%", f"{memory_avg:.0f}MB", uptime_str
                )
...
def alerts(output_format, active_only):
    ...
    try:
        ...
        stats_text.append(
            f"Rules: {stats['enabled_rules']}/{stats['rules_count']}\n", style="yellow"
        )
        stats_text.append(
            f"Notifications Sent: {stats['notifications_sent']}", style="cyan"
        )
...
def logs(project_name, tail, window, output_format):
    ...
    try:
        ...
        with console.status("[bold green]Analyzing logs..."):
            analysis_result = log_analyzer.analyze_project_logs(
                project_name, tail, window
            )
        ...
        summary_text.append(
            f"Parsed Lines: {analysis_result.parsed_lines}\n", style="cyan"
        )
        ...
        summary_text.append(
            f"Analysis Time: {time.strftime('%H:%M:%S', time.localtime(analysis_result.analysis_time))}",
            style="dim",
        )

        console.print(
            Panel(summary_text, title="Log Analysis Summary", border_style="blue")
        )
        ...
        if analysis_result.patterns_found:
            ...
            for pattern_name, count in sorted(
                analysis_result.patterns_found.items(), key=lambda x: x[1], reverse=True
            ):
                ...
                patterns_table.add_row(
                    pattern_name.replace("_", " ").title(), str(count), description
                )
...
def dashboard(project_name, refresh):
    ...
    try:
        ...
        console.print(
            f"[dim]Refresh interval: {refresh}s (Press Ctrl+C to exit)[/dim]\n"
        )
...
def background(start, stop, status, interval):
    ...
    try:
        if start:
            ...
            console.print(
                f"[green]✅ Background monitoring started (interval: {interval}s)[/green]"
            )
            console.print(
                "[dim]Use 'monitoring background --status' to check status[/dim]"
            )
            ...
            status_table.add_row(
                "Health Checker", health_status, f"{health_checks} checks performed"
            )
            ...
            status_table.add_row(
                "Metrics Collector", metrics_status, f"{metric_count} metrics tracked"
            )
            ...
            status_table.add_row(
                "Alert Manager", alerts_status, f"{rules_count} rules enabled"
            )
