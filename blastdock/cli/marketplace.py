...
@click.option(
    "--source",
    type=click.Choice(["official", "community", "all"]),
    default="all",
...
{', '.join(f"[cyan]{tag}[/cyan]" for tag in template.tags)}"""
        ...
        panel = Panel(
            info_content, title=f"Template: {template_id}", border_style="cyan"
        )
        console.print(panel)
...
def show_categories():
    ...
    try:
        ...
        for category, count in sorted(
            ...
            table.add_row(
                category.value, str(count), category_descriptions.get(category, "")
            )
        ...
        if installed:
            ...
            for category, templates in sorted(by_category.items()):
                ...
                for template in sorted(
                    templates, key=lambda t: t.downloads, reverse=True
                )[:5]:
                    console.print(
                        f"  • [cyan]{template.id}[/cyan] - {template.display_name} "
...
def uninstall_template(template_name: str):
    ...
    try:
        ...
        if result["success"]:
            console.print(
                f"[green]✅ Successfully uninstalled '{template_name}'[/green]"
            )
        else:
...
def update_template(template_name: str):
    ...
    try:
        ...
        if result["success"]:
            console.print(
                f"[green]✅ Successfully updated '{template_name}' to v{result['version']}[/green]"
            )
        else:
