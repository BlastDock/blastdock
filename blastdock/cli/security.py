...
from ..security import (
    ...
    get_docker_security_checker,
    get_template_security_scanner,
    get_config_security,
...
@security.command()
@click.option(
    "--init-encryption", is_flag=True, help="Initialize configuration encryption"
)
...
def config(init_encryption, master_password):
    ...
    status_table.add_row(
        "Encryption Enabled", "✅ Yes" if status["encryption_enabled"] else "❌ No"
    )
    status_table.add_row(
        "Key File", "✅ Exists" if status["key_file_exists"] else "❌ Missing"
    )
    status_table.add_row(
        "Salt File", "✅ Exists" if status["salt_file_exists"] else "❌ Missing"
    )
...
def guidelines():
    ...
    console.print(
        f"  • Dangerous patterns detected: {security_report['dangerous_patterns_count']}"
    )
    console.print(
        f"  • Allowed extensions: {len(security_report['allowed_extensions'])}"
    )
    console.print(
        f"  • Blocked extensions: {len(security_report['blocked_extensions'])}"
    )
...
def _display_security_scan_results(results: dict, detailed: bool):
    ...
    console.print(
        f"[bold]Overall Security Score: [{score_color}]{score_icon} {score}/100[/{score_color}][/bold]\\n"
    )
    ...
    if docker_results.get("accessible"):
        ...
        if docker_results.get("security_issues"):
            for issue in docker_results["security_issues"][:3]:  # Show top 3
                ...
                console.print(
                    f"  • [{severity.upper()}] {issue.get('issue', 'Unknown issue')}"
                )
        else:
    ...
    if template_results.get("accessible"):
        ...
        console.print(
            f"[bold]📄 Template Security: {template_score}/100 ({templates_count} templates)[/bold]"
        )
    ...
    console.print(
        f"  • Encryption: {'✅ Enabled' if config_results.get('encryption_enabled') else '❌ Disabled'}"
    )
    console.print(
        f"  • Key Management: {'✅ Configured' if config_results.get('key_file_exists') else '❌ Not configured'}"
    )
...
def _display_container_scan_results(result: dict):
    ...
    config_table.add_row(
        "Privileged", "✅ No" if not config.get("privileged") else "❌ Yes"
    )
...
def _display_file_scan_results(result: dict, fix_issues: bool):
    ...
    if fix_issues:
        ...
        for file_info in insecure_files:
            ...
            success, _ = file_ops.set_secure_permissions(
                file_path, os.path.isdir(file_path)
            )
            if success:
                fixed_count += 1

        console.print(
            f"[green]✅ Fixed {fixed_count} out of {len(insecure_files)} files[/green]"
        )
