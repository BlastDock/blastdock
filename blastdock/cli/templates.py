...
def validate(templates_dir, output, filter, save_report):
    ...
    with Progress(
        ...
        for i, template_file in enumerate(template_files):
            ...
            progress.update(
                task, description=f"Validating {template_name}...", completed=i
            )
        ...
        progress.update(
            task, completed=total_templates, description="Validation complete!"
        )
...
def enhance(templates_dir, filter, dry_run, backup):
    ...
    for name, analysis in analyses.items():
        if (
            filter == "none"
            and analysis.traefik_compatibility == TraefikCompatibility.NONE
        ):
            to_enhance.append((name, analysis))
        elif (
            filter == "basic"
            and analysis.traefik_compatibility == TraefikCompatibility.BASIC
        ):
            to_enhance.append((name, analysis))
        elif (
            filter == "partial"
            and analysis.traefik_compatibility == TraefikCompatibility.PARTIAL
        ):
            to_enhance.append((name, analysis))

    if not to_enhance:
        console.print(
            f"[green]✅ No templates need enhancement for filter: {filter}[/green]"
        )
        return
    ...
    for name, analysis in to_enhance:
        ...
        table.add_row(
            name, analysis.traefik_compatibility.value, f"{analysis.score}/100", issues
        )
    ...
    with Progress(
        ...
        for i, (name, analysis) in enumerate(to_enhance):
            ...
            # Enhance template
            success, message = validator.enhance_template_traefik_support(
                str(template_path)
            )
        ...
        progress.update(
            task, completed=len(to_enhance), description="Enhancement complete!"
        )
    ...
    if output == "table":
        ...
        for template_file in sorted(template_files):
            try:
                ...
                table.add_row(
                    template_file.stem, "Error loading template", "-", "-", "-"
                )
        ...
        for template_file in sorted(template_files):
            try:
                ...
                templates_data.append({"name": template_file.stem, "error": str(e)})
...
def _filter_analyses(analyses, filter_type):
    ...
    if filter_type == "all":
        ...
        return {
            ...
            if v.traefik_compatibility == TraefikCompatibility.NONE
        }
    return analyses
...
def _display_template_analysis(analysis):
    ...
    for category, results in results_by_category.items():
        ...
        for result in results:
            ...
            console.print(
                f"  {level_icon} [{level_color}]{result.message}[/{level_color}]"
            )
            if result.suggestion:
