...
def show_config(
    profile: str, section: Optional[str], output_format: str, no_sensitive: bool
):
...
def set_config(key: str, value: str, profile: str, value_type: str):
    ...
    try:
        ...
        console.print(
            f"[green]Set {key} = {parsed_value} in profile '{profile}'[/green]"
        )
...
def reset_config(profile: str, section: Optional[str], confirm: bool):
    ...
    try:
        if not confirm:
            sections_text = (
                f"section '{section}'" if section else "entire configuration"
            )
            if not click.confirm(
                f"Reset {sections_text} for profile '{profile}' to defaults?"
            ):
...
def export_config(
    export_path: str, profile: str, export_format: str, include_secrets: bool
):
...
def import_config(import_path: str, profile: str, merge: bool, confirm: bool):
    ...
    try:
        ...
        if not confirm:
            ...
            if not click.confirm(
                f"Import configuration will {action} existing configuration. Continue?"
            ):
...
def create_profile(profile_name: str, description: Optional[str], base: str):
    ...
    try:
        ...
        console.print(
            f"[green]Created profile '{profile_name}' based on '{base}'[/green]"
        )
...
def copy_profile(source_profile: str, target_profile: str, description: Optional[str]):
    ...
    try:
        ...
        console.print(
            f"[green]Copied profile '{source_profile}' to '{target_profile}'[/green]"
        )
...
def restore_backup(backup_file: str, profile: str, confirm: bool):
    ...
    try:
        if not confirm:
            if not click.confirm(
                f"Restore backup '{backup_file}' to profile '{profile}'? This will overwrite current configuration."
            ):
        ...
        console.print(
            f"[green]Restored backup '{backup_file}' to profile '{profile}'[/green]"
        )
