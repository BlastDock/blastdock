...
def validate_project_directory_path(
    project_dir: Path, project_name: str, base_dir: Path
) -> None:
...
class DeploymentManager:
    ...
    def deploy_project(
        ...
        if deployment_result["success"]:
            ...
            self._save_project_config(
                project_name, template_name, config_values, project_dir
            )
    ...
    def _generate_compose_file(
        self, template_data: Dict[str, Any], project_dir: Path, dry_run: bool
    ) -> Path:
    ...
    def _show_deployment_plan(
        self, project_name: str, template_name: str, template_data: Dict[str, Any]
    ):
    ...
    def _show_deployment_info(self, project_name: str, template_data: Dict[str, Any]):
        ...
        if "domain" in config and config["domain"]:
            ...
            console.print(
                f"URL: https://{config['subdomain']}.{config.get('default_domain', 'localhost')}"
            )
...
@click.option(
    "--no-enhance", is_flag=True, help="Disable automatic Traefik enhancement"
)
@click.option(
    "--security",
    default="standard",
...
def list_deployments(output_format):
    ...
    if output_format == "json":
        ...
        for project in projects:
            table.add_row(project["name"], project["status"], str(project["services"]))
...
def deployment_status(project_name):
    ...
    try:
        ...
        if not project_containers:
            console.print(
                f"[yellow]No containers found for project '{project_name}'[/yellow]"
            )
            return
...
def remove_deployment(project_name, force, keep_volumes):
    ...
    try:
        ...
        if project_dir.exists():
            ...
            if result.returncode == 0:
                console.print(
                    f"[green]✓ Project '{project_name}' removed successfully[/green]"
                )
...
def deployment_logs(project_name, follow, tail, service):
    ...
    try:
        ...
        if (
            ...
            console.print(
                f"[yellow]Warning: Docker logs command exited with code {result.returncode}[/yellow]"
            )
        ...
        with console.status("[bold green]Updating deployment...") as status:
            if pull:
                ...
                if pull_result.returncode != 0:
                    console.print(
                        f"[yellow]Warning: Failed to pull images: {pull_result.stderr}[/yellow]"
                    )

            status.update("[bold green]Recreating containers...")
            cmd = ["docker-compose", "-p", project_name, "up", "-d", "--force-recreate"]
            result = subprocess.run(
                cmd, cwd=project_dir_str, capture_output=True, text=True, timeout=300
            )

            if result.returncode == 0:
                console.print(
                    f"[green]✓ Project '{project_name}' updated successfully[/green]"
                )
            else:
    ...
    try:
        ...
        try:
            validate_project_directory_path(
                project_dir, project_name, base_projects_dir
            )
        except ValueError as e:
            console.print(f"[red]Security validation failed: {e}[/red]")
            return
            ...
            if compose_file.exists():
                ...
                with open(compose_file) as f:
                    ...
                    if services:
                        ...
                        console.print(
                            "[red]No services found in docker-compose.yml[/red]"
                        )
                        return
