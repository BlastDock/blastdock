...
@diagnostics.command()
@click.option(
    "--auto-fix", is_flag=True, help="Attempt to automatically fix detected issues"
)
@click.option(
...
def health(auto_fix, dry_run):
    ...
    for issue in issues:
        ...
        component_branch = issue_tree.add(
            f"{severity_icon} {issue['component'].replace('_', ' ').title()}"
        )
...
def _display_error_table(error_history: list):
    ...
    error_table = Table(
        title="Recent Errors", show_header=True, header_style="bold magenta"
    )
    ...
    for error in error_history:
        ...
        message = (
            ...
            if len(error.error_message) > 50
            else error.error_message
        )

        error_table.add_row(
            time_str, error_id_short, error.error_type, severity_display, message
        )
...
def _attempt_auto_fixes(issues: list):
    ...
    for issue in issues:
        ...
        if component == "docker_available":
            ...
            console.print(
                f"  [yellow]⚠️ No automatic fix available for {component}[/yellow]"
            )
