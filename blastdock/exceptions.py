...
class ProjectConfigurationError(ProjectError):
    ...
    def __init__(self, project_name: str, config_error: str):
        super().__init__(
            f"Configuration error in project '{project_name}': {config_error}"
        )
        self.project_name = project_name
...
class DockerComposeError(DockerError):
    ...
    def __init__(self, operation: str, project_name: str, error_output: str):
        super().__init__(
            f"Docker Compose {operation} failed for '{project_name}': {error_output}"
        )
...
class InsufficientSpaceError(FileSystemError):
    ...
    def __init__(self, required_space: str, available_space: str):
        super().__init__(
            f"Insufficient disk space. Required: {required_space}, Available: {available_space}"
        )
...
class TraefikNotInstalledError(TraefikError):
    ...
    def __init__(self):
        super().__init__(
            "Traefik is not installed. Install with: blastdock traefik install"
        )
...
class TraefikNotRunningError(TraefikError):
    ...
    def __init__(self):
        super().__init__(
            "Traefik is installed but not running. Start with: blastdock traefik restart"
        )
...
class DomainConflictError(DomainError):
    ...
    def __init__(self, domain: str, conflicting_project: str):
        super().__init__(
            f"Domain '{domain}' is already used by project '{conflicting_project}'"
        )
        self.domain = domain
...
class SSLValidationError(SSLError):
    ...
    def __init__(self, domain: str, validation_error: str):
        super().__init__(
            f"SSL certificate validation failed for '{domain}': {validation_error}"
        )
        self.domain = domain
...
class MigrationFailedError(MigrationError):
    ...
    def __init__(self, project_name: str, step: str, reason: str):
        super().__init__(
            f"Migration of '{project_name}' failed at step '{step}': {reason}"
        )
        self.project_name = project_name
        self.step = step
        self.reason = reason
...
class MigrationIncompatibleError(MigrationError):
    ...
    def __init__(self, project_name: str, issues: list):
        ...
        super().__init__(
            f"Project '{project_name}' is not compatible with migration: {issues_str}"
        )
        self.project_name = project_name
        self.issues = issues
