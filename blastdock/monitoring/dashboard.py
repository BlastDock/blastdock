...
class MonitoringDashboard:
    ...
    def show_project_overview(self, project_name: str) -> None:
        ...
        try:
            ...
            metrics_data = self.metrics_collector.get_project_dashboard_data(
                project_name
            )
            ...
            layout["left"].split_column(
                Layout(name="health", ratio=1), Layout(name="metrics", ratio=2)
            )

            layout["right"].split_column(
                Layout(name="alerts", ratio=1), Layout(name="containers", ratio=2)
            )

            # Header
            header_text = Text(
                f"BlastDock Monitoring Dashboard - {project_name}", style="bold blue"
            )
            ...
            # Container details
            containers_panel = self._create_containers_panel(
    ...
    def _create_health_panel(self, health_data: Dict[str, Any]) -> Panel:
        ...
        content.append(
            f"Response: {health_data.get('duration_ms', 0):.1f}ms", style="dim"
        )
    ...
    def _create_metrics_panel(self, metrics_data: Dict[str, Any]) -> Panel:
        ...
        if "error" in metrics_data:
            return Panel(
                f"Error: {metrics_data['error']}", title="Metrics", border_style="red"
            )
    ...
    def _create_alerts_panel(self, alerts: List[Any]) -> Panel:
        ...
        return Panel(
            content, title="Active Alerts", border_style="red" if alerts else "green"
        )
    ...
    def _create_containers_panel(self, services: Dict[str, Any]) -> Panel:
        ...
        for service_name, service_info in services.items():
            ...
            table.add_row(service_name, status_text, cpu_str, memory_str)
    ...
    def show_system_overview(self) -> None:
        ...
        try:
            ...
            layout["body"].split_row(Layout(name="metrics"), Layout(name="alerts"))
    ...
    def _create_system_metrics_panel(self, metrics: Dict[str, Any]) -> Panel:
        ...
        for metric_name, description in system_metrics:
            if metric_name in metrics:
                ...
                table.add_row(
                    description, str(count), metric_info.get("description", "")
                )

        return Panel(table, title="System Metrics", border_style="blue")

    def _create_system_alerts_panel(
        self, alerts: List[Any], stats: Dict[str, Any]
    ) -> Panel:
        ...
        content.append(
            f"  Active: {stats.get('active_alerts_current', 0)}\n", style="red"
        )
        content.append(f"  Total: {stats.get('total_alerts', 0)}\n", style="blue")
        content.append(
            f"  Resolved: {stats.get('resolved_alerts', 0)}\n", style="green"
        )
        content.append(
            f"  Rules: {stats.get('enabled_rules', 0)}/{stats.get('rules_count', 0)}\n\n",
            style="yellow",
        )
        ...
        return Panel(
            content, title="Alert Status", border_style="red" if alerts else "green"
        )
    ...
    def export_dashboard_data(self, project_name: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if project_name:
                ...
                export_data["health"] = self.health_checker.check_project_health(
                    project_name
                )
                export_data["metrics"] = (
                    self.metrics_collector.get_project_dashboard_data(project_name)
                )
                export_data["alerts"] = [
                    {
