...
class AlertManager:
    ...
    def _get_metric_values_for_rule(
    ...
    def _fire_alert(
        self, rule: AlertRule, metric_key: str, value: float, current_time: float
    ):
        """Fire an alert"""
    ...
    def _send_email_notification(self, alert: Alert, channel: NotificationChannel):
        ...
        if not EMAIL_AVAILABLE:
            raise ValueError(
                "Email functionality not available - missing email modules"
            )
        ...
        msg["Subject"] = (
            f"[{alert.severity.value.upper()}] BlastDock Alert: {alert.rule_name}"
        )
...
"""
    ...
    def _send_email_resolution(self, alert: Alert, channel: NotificationChannel):
        ...
        if not EMAIL_AVAILABLE:
            ...
            self.logger.debug(
                "Email notification unavailable - required modules not installed"
            )
            return
        ...
        if not all([smtp_server, username, password, from_email, to_emails]):
            ...
            self.logger.warning(
                f"Email configuration incomplete for alert {alert.rule_name}. Missing required fields."
            )
            return
...
"""
    ...
    def silence_alert(self, rule_name: str, metric_key: str, duration_seconds: int):
        ...
        with self._alerts_lock:
            if alert_id in self._active_alerts:
                ...
                self.logger.info(
                    f"Silenced alert {alert_id} for {duration_seconds} seconds"
                )
                return True
    ...
    def start_evaluation(self, interval: float = 30.0):
        ...
        self._evaluation_thread = threading.Thread(
            target=self._evaluation_loop, name="alert-evaluator", daemon=True
        )
                ...
                with self._alerts_lock:
                    for alert in self._active_alerts.values():
                        if (
                            ...
                            and current_time >= alert.silenced_until
                        ):
                            alert.status = AlertStatus.FIRING
                            alert.silenced_until = None
