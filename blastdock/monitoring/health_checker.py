...
class HealthChecker:
    ...
    def register_service_health_config(
        self, project_name: str, service_name: str, config: ServiceHealthConfig
    ):
        ...
        try:
            ...
            if container_status == "running":
                ...
                try:
                    ...
                    self.logger.debug(
                        f"Could not get container stats for {container_name}: {e}"
                    )
    ...
    def _check_service_health(
        self, config: ServiceHealthConfig, container_info: Dict[str, Any]
    ) -> HealthCheckResult:
                ...
                if not port and ports:
                    ...
                    for internal_port, mappings in ports.items():
                        if mappings and len(mappings) > 0:
                            ...
                            port = int(
                                mappings[0].get("HostPort", internal_port.split("/")[0])
                            )
                            break
    ...
    def _check_exec_health(
        ...
        try:
            ...
            return HealthCheckResult(
                ...
                timestamp=time.time(),
                suggestions=[
                    "Command may be hanging",
                    "Increase timeout or check service",
                ],
            )
        except Exception as e:
            return HealthCheckResult(
    ...
    def get_health_history(
        self, project_name: str, service_name: str = None, limit: int = 10
    ) -> List[Dict[str, Any]]:
    ...
    def get_health_statistics(self, project_name: str = None) -> Dict[str, Any]:
        ...
        healthy_checks = sum(
            1 for r in project_history if r.status == HealthStatus.HEALTHY
        )
        unhealthy_checks = sum(
            1 for r in project_history if r.status == HealthStatus.UNHEALTHY
        )
        degraded_checks = sum(
            1 for r in project_history if r.status == HealthStatus.DEGRADED
        )
        unknown_checks = sum(
            1 for r in project_history if r.status == HealthStatus.UNKNOWN
        )
        ...
        avg_response_time = (
            sum(r.response_time_ms for r in project_history) / total_checks
        )
    ...
    def start_background_monitoring(self, interval: float = 30.0):
        ...
        self._monitoring_thread = threading.Thread(
            target=self._monitoring_loop, name="health-monitor", daemon=True
        )
        self._monitoring_thread.start()

        self.logger.info(
            f"Started background health monitoring (interval: {interval}s)"
        )
