...
class MetricsCollector:
    ...
    def _initialize_core_metrics(self):
        ...
        core_metrics = [
            ...
            (
                ...
                "count",
                "Number of running containers in project",
            ),
            ...
            (
                ...
                "ms",
                "Template loading duration in milliseconds",
            ),
        ]
    ...
    def collect_container_metrics(self, project_name: str):
        ...
        try:
                ...
                if container_status == "running":
                    ...
                    try:
                        ...
                        if stats:
                            ...
                            self.record_metric(
                                "container_cpu_percent", cpu_percent, timestamp, labels
                            )
                            self.record_metric(
                            ...
                            if disk_stats:
                                self.record_metric(
                                    ...
                                    disk_stats.get("read_bytes", 0),
                                    timestamp,
                                    labels,
                                )
                                self.record_metric(
                                    "container_disk_write_bytes",
                                    disk_stats.get("write_bytes", 0),
                                    timestamp,
                                    labels,
                                )
            ...
            self.record_metric(
                "project_container_count", len(containers), timestamp, project_labels
            )
            self.record_metric(
                "project_running_containers", running_count, timestamp, project_labels
            )
            self.record_metric(
                "project_memory_total_mb",
                project_memory_total,
                timestamp,
                project_labels,
            )
            self.record_metric(
            ...
            self.logger.error(
                f"Failed to collect container metrics for {project_name}: {e}"
            )
    ...
    def collect_system_metrics(self):
        ...
        try:
            ...
            if system_info:
                self.record_metric(
                    "system_containers_total",
                    system_info.get("containers", 0),
                    timestamp,
                )
                self.record_metric(
                    "system_images_total", system_info.get("images", 0), timestamp
                )
            ...
            try:
                ...
                if networks_info:
                    ...
                    self.record_metric(
                        "system_networks_total", network_count, timestamp
                    )
            except Exception as e:
    ...
    def record_health_metric(
        self, project_name: str, service_name: str, duration_ms: float, success: bool
    ):
        """Record health check metrics"""
        timestamp = time.time()
        labels = {"project": project_name, "service": service_name}
    ...
    def record_deployment_metric(
        self, project_name: str, duration_seconds: float, success: bool
    ):
        """Record deployment metrics"""
        timestamp = time.time()
        labels = {"project": project_name, "success": str(success)}

        self.record_metric(
            "deployment_duration_seconds", duration_seconds, timestamp, labels
        )
    ...
    def record_template_metric(self, template_name: str, load_duration_ms: float):
        ...
        self.record_metric(
            "template_load_duration_ms", load_duration_ms, timestamp, labels
        )

    def calculate_health_success_rate(
        self, project_name: str, service_name: str, window_hours: int = 24
    ) -> float:
        ...
        try:
            ...
            relevant_points = [
                p
                for p in duration_metric.points
                if (
                    p.timestamp >= cutoff_time
                    and p.labels.get("project") == project_name
                    and p.labels.get("service") == service_name
                )
            ]
    ...
    def get_project_dashboard_data(
        self, project_name: str, window_hours: int = 24
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for metric_name in key_metrics:
                summary = self.get_metric_summary(
                    metric_name, start_time, end_time, project_labels
                )
                points = self.get_metric_values(
                    metric_name, start_time, end_time, project_labels
                )
            ...
            for container in containers:
                ...
                container_metrics[container_name] = {
                    "cpu": self.get_metric_summary(
                        "container_cpu_percent", start_time, end_time, container_labels
                    ),
                    "memory": self.get_metric_summary(
                        "container_memory_usage_mb",
                        start_time,
                        end_time,
                        container_labels,
                    ),
                    "uptime": self.get_metric_summary(
                        "container_uptime_seconds",
                        start_time,
                        end_time,
                        container_labels,
            ...
            self.logger.error(
                f"Error generating dashboard data for {project_name}: {e}"
            )
            return {
                "project_name": project_name,
                "error": str(e),
        ...
        with self._metrics_lock:
            return {
                name: {
                    ...
                    "labels": list(
                        set().union(*(p.labels.keys() for p in metric.points))
                    ),
                    "latest_timestamp": max(
                        (p.timestamp for p in metric.points), default=0
                    ),
                }
    ...
    def export_metrics(
        self, format: str = "json", start_time: float = None, end_time: float = None
    ) -> str:
        ...
        with self._metrics_lock:
            for name, metric in self._metrics.items():
                ...
                for point in points:
                    ...
                    if (
                        label_key not in latest_points
                        or point.timestamp > latest_points[label_key].timestamp
                    ):
