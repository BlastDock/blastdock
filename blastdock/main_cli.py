...
def setup_cli_environment(
    ...
    # Initialize logging
    initialize_logging(log_level=level, log_to_console=not quiet, log_to_file=True)
    ...
    try:
        ...
        if not quiet:
            console.print(
                f"[yellow]Warning: Could not initialize directories: {e}[/yellow]"
            )
...
@click.option(
    "--interactive", "-i", is_flag=True, help="Interactive configuration mode"
)
...
def init(ctx, template_name, interactive, name, traefik):
    ...
    console.print(
        "[yellow]Note: 'init' is deprecated. Use 'blastdock deploy create' instead[/yellow]"
    )
    ctx.invoke(
        deploy_group.commands["create"],
...
def list(output_format):
    ...
    console.print(
        "[yellow]Note: 'list' is deprecated. Use 'blastdock deploy list' instead[/yellow]"
    )
...
def templates(ctx):
    ...
    console.print(
        "[yellow]Note: 'templates' is deprecated. Use 'blastdock marketplace search' instead[/yellow]"
    )
...
@click.option(
    "--dashboard/--no-dashboard", default=True, help="Enable Traefik dashboard"
)
...
@click.option(
    "--all", "migrate_all", is_flag=True, help="Migrate all compatible projects"
)
