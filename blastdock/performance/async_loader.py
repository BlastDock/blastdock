...
class AsyncTemplateLoader:
    ...
    def __init__(self, max_workers: int = 8, max_queue_size: int = 1000):
        ...
        self.logger.info(
            f"Async template loader initialized with {max_workers} workers"
        )
    ...
    async def load_templates_batch(
        self, templates: List[Tuple[str, str]], priority: int = 1
    ) -> Dict[str, LoadResult]:
    ...
    async def preload_popular_templates(
        self, template_registry, top_n: int = 20
    ) -> Dict[str, LoadResult]:
    ...
    async def _process_task(self, task: LoadTask, worker_id: int) -> LoadResult:
        ...
        try:
            ...
            processed_data = await self._process_template_data(
                template_data, task.template_path
            )
    ...
    async def _validate_template_structure(
        ...
        if "services" not in compose or not compose["services"]:
            return {"valid": False, "error": "No services defined in compose section"}

        return {"valid": True}

    async def _process_template_data(
        self, template_data: Dict[str, Any], template_path: str
    ) -> Dict[str, Any]:
    ...
    async def _check_cache(
        self, template_name: str, template_path: str
    ) -> Optional[LoadResult]:
    ...
    async def _cache_result(
        self, template_name: str, template_data: Dict[str, Any], load_time: float
    ):
        ...
        if source_path.exists():
            ...
            self.logger.warning(
                f"Template file no longer exists during caching: {source_path}"
            )
        ...
        self.cache_manager.set(
            cache_key, template_data, ttl=3600, tags=["async_template", template_name]
        )
    ...
    async def _wait_for_completion(
        self, template_name: str, timeout: float = 60.0
    ) -> LoadResult:
        ...
        # Timeout
        return LoadResult(
            template_name=template_name, success=False, error="Loading timeout"
        )
    ...
    async def _optimize_loading_order(
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        worker_stats_summary = {
            "total_tasks": sum(
                stats["tasks_processed"] for stats in self._worker_stats.values()
            ),
            "total_errors": sum(
                stats["errors"] for stats in self._worker_stats.values()
            ),
            "avg_processing_time": (
                sum(stats["avg_time"] for stats in self._worker_stats.values())
...
async def get_async_loader() -> AsyncTemplateLoader:
    ...
    if _async_loader is None:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to initialize async template loader: {e}"
            ) from e
