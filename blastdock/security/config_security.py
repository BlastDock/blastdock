...
class ConfigurationSecurity:
    ...
    def secure_config(
        self, config: Dict[str, Any], encrypt_sensitive: bool = True
    ) -> Dict[str, Any]:
    ...
    def save_secure_config(
        self, config: Dict[str, Any], file_path: str, encrypt_sensitive: bool = True
    ) -> bool:
    ...
    def validate_config_security(
        ...
        def check_secrets(obj: Any, path: str = "") -> None:
            if isinstance(obj, dict):
                ...
                if self._is_sensitive_key(path.split(".")[-1]) and not obj.startswith(
                    "encrypted:"
                ):
