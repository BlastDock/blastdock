...
class SecurityValidator:
    ...
    def validate_project_name(self, project_name: str) -> Tuple[bool, Optional[str]]:
        ...
        if not self.PROJECT_NAME_PATTERN.match(project_name):
            return (
                False,
                "Project name contains invalid characters (only a-z, A-Z, 0-9, _, - allowed)",
            )
    ...
    def validate_template_name(self, template_name: str) -> Tuple[bool, Optional[str]]:
        ...
        for pattern in self.DANGEROUS_PATTERNS:
            if re.search(pattern, template_name, re.IGNORECASE):
                return (
                    False,
                    f"Template name contains potentially dangerous pattern: {pattern}",
                )
    ...
    def validate_environment_variable(
        ...
        for pattern in self.DANGEROUS_PATTERNS:
            if re.search(pattern, value, re.IGNORECASE):
                return (
                    False,
                    f"Environment variable value contains dangerous pattern: {pattern}",
                )
