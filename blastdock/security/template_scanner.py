...
class TemplateSecurityScanner:
    ...
    def _scan_template_directory(self, template_dir: str) -> Dict[str, Any]:
        ...
        has_compose = any(
            os.path.exists(os.path.join(template_dir, f)) for f in required_files
        )
    ...
    def _scan_template_file(self, file_path: str) -> Dict[str, Any]:
        ...
        perm_valid, perm_error = self.security_validator.check_file_permissions(
            file_path
        )
        if not perm_valid:
            security_issues.append(
        ...
        for pattern in self.SENSITIVE_ENV_PATTERNS:
            if pattern in name_lower:
                ...
                if (
                    ...
                    and not value.startswith("{{")
                    and value not in ["changeme", "secret", "password", "example"]
                ):
                    return True
    ...
    def _generate_security_summary(
        self, issues: List[Dict[str, Any]], templates_count: int
    ) -> List[str]:
        ...
        summary.append(
            f"🔍 Found {len(issues)} security issues across {templates_count} templates"
        )
