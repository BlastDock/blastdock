...
class TemplateValidator:
    ...
    def __init__(self, templates_dir: str = None):
        ...
        self.required_sections = ["template_info", "fields", "compose"]
        self.required_template_info = ["description", "version", "services"]
    ...
    def validate_template(self, template_path: str) -> TemplateAnalysis:
        ...
        try:
            ...
            if "template_info" in template_data:
                results.extend(
                    self._validate_template_info(template_data["template_info"])
                )
            ...
            results.extend(
                self._validate_traefik_config(template_data, traefik_compatibility)
            )
    ...
    def _analyze_traefik_compatibility(
    ...
    def _validate_traefik_config(
        self, template_data: Dict, compatibility: TraefikCompatibility
    ) -> List[ValidationResult]:
        ...
        if compatibility == TraefikCompatibility.NONE:
            ...
            has_security_headers = any(
                "headers" in middleware.get("config", {}) for middleware in middlewares
            )
    ...
    def _check_common_issues(
        self, template_data: Dict, content: str
    ) -> List[ValidationResult]:
    ...
    def enhance_template_traefik_support(
        self,
        template_path: str,
        ...
        try:
            ...
            from ..performance.traefik_enhancer import (
                get_traefik_enhancer,
                SecurityLevel,
            )
...
"""
        ...
        for analysis in sorted_templates[:10]:  # Bottom 10 templates
            if analysis.error_count > 0 or analysis.warning_count > 5:
                ...
                # Show top 3 issues
                top_issues = [
                    r for r in analysis.results if r.level == ValidationLevel.ERROR
                ][:3]
                if top_issues:
                    report += "- Top Issues:\n"
