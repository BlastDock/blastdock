...
class ErrorRecoveryEngine:
    ...
    def execute_recovery_plan(
        self, plan: RecoveryPlan, dry_run: bool = False, auto_confirm: bool = False
    ) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"Starting recovery plan for {plan.error_type} (dry_run={dry_run})"
        )
        ...
        try:
            for i, step in enumerate(plan.steps):
                ...
                if step_result["success"]:
                    ...
                    self.logger.info(
                        f"Recovery step {i+1} completed: {step.description}"
                    )
                else:
    ...
    def _execute_recovery_step(
        self, step: RecoveryStep, dry_run: bool = False, auto_confirm: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if step.requires_confirmation and not auto_confirm:
                self.logger.warning(
                    f"Manual confirmation required for: {step.description}"
                )
    ...
    def get_recovery_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_execution_time": sum(
                r["execution_time"] for r in self.recovery_history
            )
            / total_attempts,
...
def attempt_auto_recovery(
    error_context: ErrorContext, dry_run: bool = False, auto_confirm: bool = False
    ...
    return recovery_engine.execute_recovery_plan(
        recovery_plan, dry_run=dry_run, auto_confirm=auto_confirm
    )
