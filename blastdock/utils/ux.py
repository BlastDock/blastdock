...
class EnhancedProgress:
    ...
    def multi_task_display(self, title: str = "BlastDock Operations"):
        ...
        # Header
        header = Panel(Align.center(Text(title, style="bold blue")), style="blue")
...
class TaskController:
    ...
    def update(self, advance: int = 1, description: str = None, **kwargs):
        ...
        self.progress.update(
            self.progress_task_id, advance=advance, description=description, **kwargs
        )
...
class MultiTaskController:
    ...
    def add_task_display(self, task_type: TaskType, title: str = None) -> Progress:
        ...
        if current_tasks == 0:
            ...
            # Split tasks section
            task_layouts = [
                Panel(prog, title=f"{name.title()} Tasks")
...
class UXManager:
    ...
    def show_welcome(self, version: str = "1.0.0"):
        ...
        welcome_text.append("\n")
        welcome_text.append(
            "Enterprise Docker Deployment Platform", style="italic cyan"
        )

        panel = Panel(Align.center(welcome_text), style="blue", padding=(1, 2))
    ...
    def show_error(
        self, message: str, error_details: str = None, suggestions: List[str] = None
    ):
    ...
    def show_info(
        self, title: str, data: Dict[str, Any], highlight_keys: List[str] = None
    ):
        ...
        for key, value in data.items():
            key_style = (
                "bold cyan" if highlight_keys and key in highlight_keys else "cyan"
            )
    ...
    def prompt_choice(
        self, message: str, choices: List[str], default: str = None
    ) -> str:
    ...
    def prompt_text(
        self, message: str, default: str = None, password: bool = False
    ) -> str:
    ...
    def deployment_progress(self, project_name: str, services: List[str]):
        ...
        with self.progress.multi_task_display(title) as controller:
            ...
            deploy_progress = controller.add_task_display(
                TaskType.DEPLOYMENT, "Deployment Progress"
            )

            yield DeploymentProgressController(deploy_progress, services, self.console)

    def show_deployment_summary(
        self, project_name: str, services: Dict[str, str], duration: float
    ):
...
class DeploymentProgressController:
    ...
    def __init__(self, progress: Progress, services: List[str], console: Console):
        ...
        for service in services:
            task_id = self.progress.add_task(f"Preparing {service}...", total=100)
...
def task_progress(
    ...
    with get_ux_manager().progress.task(
        task_id, name, task_type, total, description
    ) as controller:
        yield controller
