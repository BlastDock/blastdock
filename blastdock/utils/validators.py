...
from .helpers import is_port_available
...
class InputValidator:
    ...
    @classmethod
    def validate_domain(
        cls, domain: str, allow_empty: bool = True, raise_on_error: bool = False
    ) -> Tuple[bool, str]:
        ...
        try:
            ...
            if len(domain) > 253:
                raise DomainValidationError(
                    domain, "Domain name too long (max 253 characters)"
                )
            ...
            for part in parts:
                if len(part) > 63:
                    raise DomainValidationError(
                        domain, f"Domain label '{part}' too long (max 63 characters)"
                    )
                if part.startswith("-") or part.endswith("-"):
                    raise DomainValidationError(
                        domain, f"Domain label '{part}' cannot start or end with hyphen"
                    )
    ...
    @classmethod
    def validate_email(
        cls, email: str, allow_empty: bool = True, raise_on_error: bool = False
    ) -> Tuple[bool, str]:
        ...
        try:
            ...
            if not allow_privileged and port_num < 1024:
                raise PortValidationError(
                    str(port), "Privileged ports (< 1024) require special permissions"
                )
    ...
    def validate_password(
        ...
        try:
            ...
            if checks:
                raise PasswordValidationError(
                    f"Password must contain {', '.join(checks)}"
                )
    ...
    def validate_database_name(
        cls, name: str, db_type: str = "mysql", raise_on_error: bool = False
    ) -> Tuple[bool, str]:
        ...
        try:
            ...
            if len(name) > max_length:
                raise DatabaseNameValidationError(
                    name,
                    f"Database name too long for {db_type} (max {max_length} characters)",
                )
            ...
            if name.lower() in cls.RESERVED_NAMES["database"]:
                raise DatabaseNameValidationError(
                    name, f"'{name}' is a reserved database name"
                )
            ...
            if db_type.lower() == "mysql":
                if name.lower().startswith("mysql"):
                    raise DatabaseNameValidationError(
                        name, "MySQL database names cannot start with 'mysql'"
                    )

            elif db_type.lower() == "postgresql":
                if name.lower().startswith("pg_"):
                    raise DatabaseNameValidationError(
                        name, "PostgreSQL database names cannot start with 'pg_'"
                    )
    ...
    def validate_ip_address(
        cls, ip: str, version: Optional[int] = None, raise_on_error: bool = False
    ) -> Tuple[bool, str]:
        """Validate IP address"""
        try:
            if not ip:
    ...
    @classmethod
    def sanitize_name(
        cls, name: str, max_length: int = 50, allowed_chars: str = "a-zA-Z0-9_-"
    ) -> str:
...
def validate_port_input(
    port: Union[str, int], check_availability: bool = True
) -> Tuple[bool, str]:
