...
class EnhancedErrorHandler:
    ...
    def _display_error_message(self, error_context: ErrorContext, exception: Exception):
        ...
        error_content.append("")
        error_content.append(
            f"[dim]Error ID:[/dim] [cyan]{error_context.error_id}[/cyan]"
        )
    ...
    def _display_diagnostics(self, error_context: ErrorContext):
        ...
        if error_context.traefik_status:
            ...
            if traefik_status.get("running"):
                ...
                status_table.add_row("Traefik", "⚠️ Installed", "Not running")
            else:
                status_table.add_row("Traefik", "❌ Not Installed", "")
        ...
        if error_context.system_info:
            system_info = []
            system_info.append(
                f"Platform: {error_context.system_info.get('platform', 'Unknown')}"
            )
    ...
    def display_error_summary(self, errors: List[ErrorContext]):
        ...
        for error in errors[-10:]:  # Show last 10 errors
            ...
            # Truncate message
            message = (
                error.error_message[:60] + "..."
                if len(error.error_message) > 60
                else error.error_message
            )

            summary_table.add_row(time_str, error.error_type, severity_display, message)
...
def handle_cli_error(
    ...
    error_handler = EnhancedErrorHandler(
        show_diagnostics=show_diagnostics, auto_report=auto_report
    )
...
def get_recovery_suggestions(exception: Exception) -> List[str]:
    ...
    suggestions = {
        ...
        TraefikNotRunningError: ["Start Traefik: blastdock traefik restart"],
        TemplateNotFoundError: [
