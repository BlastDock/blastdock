        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if track_time:
                    ...
                    if (
                        duration > 1.0
                    ):  # Only show for operations taking more than 1 second
                        ux.console.print(
                ...
                ux.show_error(
                    f"Command failed: {func.__name__}", error_msg, suggestions
                )
                raise
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            if confirm_destructive:
                ...
                if any(
                    keyword in func.__name__.lower() for keyword in destructive_keywords
                ):
...
class ProgressSteps:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.progress_controller:
            if exc_type is None:
                ...
                self.progress_controller.fail(
                    f"Failed at step: {self.get_current_step_name()}"
                )
    ...
    def next_step(self, details: str = ""):
        ...
        if self.current_step < self.total_steps:
            ...
            self.progress_controller.update(advance=100, description=description)
            self.current_step += 1
    ...
    def update_current_step(self, details: str, progress_increment: int = 0):
        ...
        if self.current_step < self.total_steps:
            ...
            self.progress_controller.update(
                advance=progress_increment, description=description
            )
