...
class Monitor:
    ...
    def get_status(self, project_name):
        ...
        try:
            ...
            self.logger.error(
                f"Unexpected error getting status for {project_name}: {e}"
            )
            return "Error"
    ...
    def get_detailed_status(self, project_name):
        ...
        try:
            ...
            status_text.append(
                f"Template: {metadata.get('template', 'unknown')}\n", style="blue"
            )
            status_text.append(
                f"Created: {self.deployment_manager.get_project_created_date(project_name)}\n\n",
                style="dim",
            )
                        ...
                        if external_mappings:
                            for mapping in external_mappings:
                                port_mappings.append(
                                    f"{mapping['HostPort']}:{internal_port}"
                                )
    ...
    def get_resource_usage(self, project_name):
        ...
        try:
            ...
            self.logger.error(
                f"Docker error getting resource usage for {project_name}: {e}"
            )
            return {"container_count": 0, "running_count": 0}
        except Exception as e:
            # BUG-005 FIX: Log unexpected errors to aid debugging
            self.logger.error(
                f"Unexpected error getting resource usage for {project_name}: {e}"
            )
            return {"container_count": 0, "running_count": 0}
    ...
    def get_logs(self, project_name, service=None, tail=50):
        ...
        return self.deployment_manager.show_logs(
            project_name, follow=False, service=service
        )
    ...
    def follow_logs(self, project_name, service=None):
        ...
        return self.deployment_manager.show_logs(
            project_name, follow=True, service=service
        )
