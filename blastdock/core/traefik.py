...
class TraefikIntegrator:
    ...
    def process_compose(
        ...
        if not template_info.get("traefik_compatible", False):
            logger.debug(
                f"Template not marked as Traefik compatible for project {project_name}"
            )
            return compose_data
    ...
    def _is_traefik_enabled(
        self, user_config: Dict[str, Any], template_data: Dict[str, Any]
    ) -> bool:
    ...
    def _add_traefik_network(self, compose: Dict[str, Any]) -> None:
        ...
        compose["networks"][self.traefik_network] = {"external": True}
    ...
    def _get_web_service(
        ...
        if (
            template_services
            and isinstance(template_services, list)
            and len(template_services) > 0
        ):
        ...
        for service_name, service_config in services.items():
            ...
            if any(
                ...
                for db in ["mysql", "postgres", "redis", "mongo", "mariadb"]
            ):
                continue
            ...
            for port in ports:
                port_str = str(port)
                if any(
                    web_port in port_str
                    for web_port in ["80:", "443:", "8080:", "3000:", "2368:"]
                ):
                    return service_name
    ...
    def _get_domain_config(
        self, project_name: str, user_config: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _inject_traefik_labels(
        ...
        if domain_config.get("tls", True):
            ...
            labels.append(
                f"traefik.http.routers.{router_name}-secure.tls.certresolver={cert_resolver}"
            )
        ...
        if domain_config.get("tls", True):
            labels.append(
                f"traefik.http.routers.{router_name}-secure.priority={priority}"
            )
    ...
    def _remove_host_ports(
        self, service: Dict[str, Any], traefik_config: Dict[str, Any]
    ) -> None:
    ...
    def _enable_traefik_for_service(
        self, service_config, service_name, project_name, user_config, port
    ):
        ...
        labels = self._generate_traefik_labels(
            service_name, project_name, user_config, port
        )
    ...
    def _generate_traefik_labels(self, service_name, project_name, config, port):
        ...
        labels = {
            ...
            f"traefik.http.services.{project_name}-{service_name}.loadbalancer.server.port": str(
                port
            ),
        }
