...
class TemplateRepository:
    ...
    def package_template(
        ...
        self.logger.info(
            f"Packaged template {template_id} v{version} ({package_size} bytes)"
        )
    ...
    def download_template(
        self, template_id: str, version: str = "latest", destination: Path = None
    ) -> Optional[Path]:
        ...
        with tarfile.open(package_file, "r:gz") as tar:
            ...
            for member in tar.getmembers():
                ...
                if not member_path.startswith(dest_realpath):
                    ...
                    raise ValueError(
                        f"Path traversal attempt in template package: {member.name}"
                    )
        ...
        self.logger.info(
            f"Downloaded template {template_id} v{package.version} to {extracted_path}"
        )
    ...
    def import_from_directory(
        self, templates_dir: Path, version: str = "1.0.0"
    ) -> Dict[str, bool]:
            ...
            try:
                ...
                for template_id, versions in data.items():
                    ...
                    for version, package_data in versions.items():
                        self.index[template_id][version] = TemplatePackage(
                            **package_data
                        )

                self.logger.info(
                    f"Loaded repository index with {len(self.index)} templates"
                )
