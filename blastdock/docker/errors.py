...
class DockerConnectionError(DockerError):
    ...
    def __init__(
        self,
        message: str = "Failed to connect to Docker daemon",
...
class ContainerError(DockerError):
    ...
    def __init__(
        ...
        if exit_code:
            ...
            if exit_code == 125:
                suggestions.append(
                    "Container configuration error - check Dockerfile and run command"
                )
            elif exit_code == 126:
                suggestions.append(
                    "Container command not executable - check command permissions"
                )
            elif exit_code == 127:
                suggestions.append(
                    "Container command not found - check command path and availability"
                )
...
def create_docker_error(
    error: Exception,
    operation: str = "Docker operation",
    ...
    if "permission denied" in error_str:
        return DockerConnectionError(
            f"{operation} failed due to permission error", str(error)
        )

    elif "connection refused" in error_str or "cannot connect" in error_str:
        return DockerNotRunningError(
            f"{operation} failed - Docker daemon not accessible"
        )
        ...
        return ImageError(
            f"{operation} failed", context.get("image_name"), context.get("registry")
        )

    elif "network" in operation.lower():
        return NetworkError(f"{operation} failed", context.get("network_name"))
