...
class NetworkManager:
    ...
    def get_network_info(self, network_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                raise NetworkError(
                    f"Failed to parse network JSON data: {e}", network_name=network_name
                )
            ...
            raise NetworkError(
                f"Failed to get network information: {e}", network_name=network_name
            )
    ...
    def create_network(
        ...
        try:
            ...
            raise NetworkError(f"Failed to create network {name}", network_name=name)

    def remove_network(self, network_name: str, force: bool = False) -> Dict[str, Any]:
        """Remove a network"""
        remove_result = {"success": False, "network_name": network_name, "errors": []}
        ...
        try:
            ...
            raise NetworkError(
                f"Failed to remove network {network_name}", network_name=network_name
            )
    ...
    def disconnect_container(
        self, network_name: str, container_name: str, force: bool = False
    ) -> Dict[str, Any]:
    ...
    def check_network_connectivity(
        self, network_name: str, test_containers: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(containers) < 2:
                connectivity_result["errors"].append(
                    "Not enough containers to test connectivity"
                )
            ...
            for i, container_a in enumerate(containers):
                ...
                for j, container_b in enumerate(containers):
                    ...
                    container_b_name = container_b.get(
                        "name", container_b.get("id", "")
                    )
                    ...
                    try:
                        ...
                        if (
                            reachable
                            and container_b_name
                            not in connectivity_result["reachable_containers"]
                        ):
                            connectivity_result["reachable_containers"].append(
                                container_b_name
                            )
                        elif (
                            not reachable
                            and container_b_name
                            not in connectivity_result["unreachable_containers"]
                        ):
                            connectivity_result["unreachable_containers"].append(
                                container_b_name
                            )
    ...
    def get_network_usage_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for network in networks:
                ...
                stats["networks_by_driver"][driver] = (
                    stats["networks_by_driver"].get(driver, 0) + 1
                )

                # Count by scope
                scope = network.get("Scope", "unknown")
                stats["networks_by_scope"][scope] = (
                    stats["networks_by_scope"].get(scope, 0) + 1
                )
                ...
                try:
                    ...
                    if container_count > 0:
                        ...
                        stats["container_count_by_network"][
                            network.get("Name", "")
                        ] = container_count

                except Exception as e:
                    self.logger.warning(
                        f"Failed to get detailed info for network {network.get('Name', '')}: {e}"
                    )
