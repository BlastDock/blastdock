...
class ImageManager:
    ...
    def pull_image(
        ...
        try:
            ...
            try:
                ...
                pull_result["errors"].append(
                    f"Could not get image info after pull: {str(e)}"
                )
    ...
    def push_image(self, image_name: str, tag: str = "latest") -> Dict[str, Any]:
        ...
        try:
            result = self.docker_client.execute_command(
                ["docker", "push", full_image_name], timeout=900
            )  # 15 minute timeout
        ...
        try:
            ...
            try:
                ...
                build_result["errors"].append(
                    f"Could not get image info after build: {str(e)}"
                )
            ...
            raise ImageError(
                f"Failed to build image {full_image_name}", image_name=full_image_name
            )

    def remove_image(
        self, image_name: str, force: bool = False, no_prune: bool = False
    ) -> Dict[str, Any]:
        """Remove an image"""
        remove_result = {
            "success": False,
        ...
        try:
            ...
            raise ImageError(
                f"Failed to remove image {image_name}", image_name=image_name
            )

    def tag_image(
        self, source_image: str, target_image: str, target_tag: str = "latest"
    ) -> Dict[str, Any]:
        """Tag an image"""
    ...
    def search_images(
        self, term: str, limit: int = 25, filters: Optional[Dict[str, str]] = None
    ) -> List[Dict[str, Any]]:
    ...
    def save_image(self, image_name: str, output_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ImageError(
                f"Failed to save image {image_name}", image_name=image_name
            )
        ...
        try:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    scan_result["vulnerabilities"] = scan_data.get(
                        "vulnerabilities", []
                    )
                    scan_result["summary"] = scan_data.get("summary", {})
                else:
                    scan_result["errors"].append(
                        "Docker Scout not available or not configured"
                    )
