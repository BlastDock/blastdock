...
class DockerClient:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - 1:
                    ...
                    self.logger.warning(
                        f"Command timeout, retrying (attempt {attempt + 1})"
                    )
                ...
                if attempt == self.max_retries - 1:
                    self.logger.error(
                        f"Command failed: {safe_cmd}, exit code: {e.returncode}"
                    )
                    ...
                    try:
                        ...
                        if len(parts) > 2:
                            ...
                            availability["warnings"].append(
                                "Could not parse Docker version format"
                            )
                    except (IndexError, ValueError):
                        availability["warnings"].append(
                            "Could not parse Docker version"
                        )
            else:
            ...
            if result.returncode == 0:
                ...
                if "version" in version_line:
                    try:
                        ...
                        availability["warnings"].append(
                            "Could not parse Docker Compose version"
                        )
            else:
    ...
    def execute_compose_command(
        ...
        if self._docker_compose_version and self._docker_compose_version.startswith(
            "v"
        ):
    ...
    def cleanup_resources(self, aggressive: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if "Total reclaimed space" in result.stdout:
                ...
                space_match = re.search(
                    r"Total reclaimed space: ([\d.]+\w+)", result.stdout
                )
                if space_match:
            ...
            container_match = re.search(
                r"Deleted Containers:\n(.+?)(?:\n\n|\Z)", result.stdout, re.DOTALL
            )
            if container_match:
                cleanup_results["containers_removed"] = len(
                    container_match.group(1).strip().split("\n")
                )
        ...
        try:
            ...
            if image_match:
                cleanup_results["images_removed"] = len(
                    image_match.group(1).strip().split("\n")
                )
        ...
        try:
            ...
            if volume_match:
                cleanup_results["volumes_removed"] = len(
                    volume_match.group(1).strip().split("\n")
                )
        ...
        try:
            ...
            if network_match:
                cleanup_results["networks_removed"] = len(
                    network_match.group(1).strip().split("\n")
                )
