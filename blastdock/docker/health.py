...
class DockerHealthChecker:
        ...
        try:
            ...
            if response_time > 5.0:
                ...
                health_report["recommendations"].append(
                    "Check system resources and Docker daemon logs"
                )
            ...
            if "system" in system_info:
                ...
                if containers_total > 50:
                    health_report["recommendations"].append(
                        "Consider cleaning up unused containers"
                    )

                if containers_running > 20:
                    health_report["issues"].append(
                        f"High number of running containers: {containers_running}"
                    )
                    health_report["recommendations"].append(
                        "Monitor resource usage and consider scaling limits"
                    )
            ...
            health_report["healthy"] = (
                health_report["daemon_responsive"]
                and len(health_report["issues"]) == 0
        ...
        try:
            ...
            try:
                ...
                health_info["issues"].append(
                    f"Failed to parse container inspect output: {e}"
                )
            ...
            if health_config:
                ...
                if health_state:
                    ...
                    if health_state.get("Status") != "healthy":
                        health_info["issues"].append(
                            f"Container health check status: {health_state.get('Status')}"
                        )
            else:
                health_info["recommendations"].append(
                    "Consider adding health check to container"
                )
            ...
            if restart_count > 5:
                ...
                health_info["recommendations"].append(
                    "Check container logs for recurring issues"
                )
    ...
    def check_compose_project_health(
        self, project_name: str, compose_file: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        try:
                ...
                if container_name:
                    try:
                        ...
                        service_health.update(
                            {
                                "resource_usage": container_health.get(
                                    "resource_usage", {}
                                ),
                                "health_checks": container_health.get(
                                    "health_checks", []
                                ),
                            }
                        )
                        service_health["issues"].extend(
                            container_health.get("issues", [])
                        )
                    except Exception as e:
                        service_health["issues"].append(
                            f"Could not check container health: {str(e)}"
                        )
            ...
            self.logger.error(
                f"Compose project health check failed for {project_name}: {e}"
            )
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            summary["overall_healthy"] = (
                summary["daemon_health"].get("healthy", False)
                and len(summary["daemon_health"].get("issues", [])) == 0
            )

            # Collect recommendations
            summary["recommendations"].extend(
                summary["daemon_health"].get("recommendations", [])
            )

        except Exception as e:
            summary["recommendations"].append(
                f"Health summary generation failed: {str(e)}"
            )
            self.logger.error(f"Health summary generation failed: {e}")

        return summary

    def monitor_container_performance(
        self, container_id: str, duration: int = 60
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            while time.time() - start_time < duration:
                try:
                    ...
                    sample = {
                        ...
                        "memory_percent": self._parse_percentage(
                            stats.get("MemPerc", "0%")
                        ),
                        "memory_usage": stats.get("MemUsage", "0B / 0B"),
                        "network_io": stats.get("NetIO", "0B / 0B"),
            ...
            if performance_data["samples"]:
                ...
                memory_values = [
                    s["memory_percent"] for s in performance_data["samples"]
                ]
                ...
                if len(cpu_values) >= 2:
                    ...
                    performance_data["trends"] = {
                        "cpu_trend": (
                            "increasing"
                            if cpu_trend > 5
                            else "decreasing" if cpu_trend < -5 else "stable"
                        ),
                        "memory_trend": (
                            "increasing"
                            if memory_trend > 5
                            else "decreasing" if memory_trend < -5 else "stable"
                        ),
                    }
