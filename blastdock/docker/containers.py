        ...
        try:
            ...
            while wait_time < max_wait:
                ...
                if state == "running":
                    ...
                    start_result["errors"].append(
                        f"Container failed to start, state: {state}"
                    )
                    break
            ...
            if wait_time >= max_wait:
                start_result["errors"].append(
                    f"Container start timeout after {max_wait} seconds"
                )

            start_result["start_time"] = time.time() - start_time
            start_result["final_state"] = container_info["state"].get(
                "Status", "unknown"
            )
        ...
        try:
            ...
            if not stop_result["success"]:
                stop_result["errors"].append(
                    f"Container did not stop, state: {final_state}"
                )
        ...
        try:
            ...
            while wait_time < max_wait:
                ...
                if state == "running":
                    ...
                    restart_result["errors"].append(
                        f"Container failed to restart, state: {state}"
                    )
                    break
            ...
            if wait_time >= max_wait:
                restart_result["errors"].append(
                    f"Container restart timeout after {max_wait} seconds"
                )

            restart_result["restart_time"] = time.time() - start_time
            restart_result["final_state"] = container_info["state"].get(
                "Status", "unknown"
            )
