...
class ComposeManager:
    ...
    def validate_compose_file(self, compose_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if version:
                ...
                try:
                    ...
                    if version_num < 3.0:
                        validation["warnings"].append(
                            f"Compose file version {version} is outdated"
                        )
                        validation["recommendations"].append(
                            "Consider upgrading to version 3.8 or higher"
                        )
                except ValueError:
                ...
                validation["recommendations"].append(
                    "Add 'version: \"3.8\"' to your compose file"
                )
            ...
            for service_name, service_config in services.items():
                ...
                if "healthcheck" not in service_config:
                    validation["recommendations"].append(
                        f"Consider adding health check for service: {service_name}"
                    )

                # Check restart policy
                if "restart" not in service_config:
                    validation["recommendations"].append(
                        f"Consider adding restart policy for service: {service_name}"
                    )
    ...
    def start_services(
        ...
        try:
            ...
            if services:
                ...
                start_result["services_started"] = list(
                    start_result["container_info"].keys()
                )
    ...
    def scale_service(
        self, service: str, replicas: int, compose_file: Optional[str] = None
    ) -> Dict[str, Any]:
